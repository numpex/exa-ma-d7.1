            ...
            \begin{tabular}{lp{.3\textwidth}p{.5\textwidth}}
                ...
                \rowcolor{white}Languages
                 & \begin{tabular}{l}
                       Python \\
                   \end{tabular}
                 & Scimba is written in \texttt{Python}, using the \texttt{JAX} and \texttt{pytorch} libraries for ML and differentiable computing.                      \\
                \rowcolor{numpexlightergray}Parallelism
                 & \begin{tabular}{l}
                       GPU, CPU (shared) \\
                   \end{tabular}
                 & Scimba takes advantage of the intrinsic parallelization capabilities of  \texttt{JAX} and  \texttt{pytorch} (shared-memory CPU and GPU, by default).  \\
                ...
                \rowcolor{white}Testing
                 & \begin{tabular}{l}
                       pytest \\
                   \end{tabular}
                 & Tests are provided for core functionalities, available by running \texttt{pytest}. Regression tests and benchmarks are in the process of being added. \\
...
It also implement continuous and discontinuous finite element solvers on unstructured quadrangle meshes, as well as their time-dependent counterparts.
...
Exa-MA has supported the internships of Marie Sengler (Strasbourg) and Daria Hrebenshchykova (Sophia-Antipolis, who went on to work as a PhD student, also supported by Exa-MA), who both worked with Scimba.
Exa-MA has also supported the PhD of Amaury Bélières-Frendo (Strasbourg), who worked on Physics-informed shape optimization with neural networks (Section~\ref{sec:app:specs:app-scimba-shape-opt-nn}).
