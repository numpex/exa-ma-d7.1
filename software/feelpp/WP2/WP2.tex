...
\paragraph{Benchmarking Tools Used:} We used \Feelpp to monitor the time spent in the various section of the code, including the reduced basis generation and the non-intrusive reduced basis method. We measured the execution time, memory usage, and FLOPS to assess the software's performance.

\paragraph{Input/Output Dataset Description}

\begin{description}
    \item[Input Data:] Describe the input dataset (size, format, data type) and provide a DOI or link to access it.
    \item[Output Data:] Specify the structure of the results (e.g., memory usage, runtime logs) and how they can be accessed or replicated.
    \item[Data Repository:] Indicate where the data is stored (e.g., Zenodo, institutional repository) and provide a DOI or URL for accessing the data.
\end{description}
