...
\label{sec:Feelpp:apps}
Feel++ powers several applications documented in Chapter~\ref{chap:applications}:
\begin{itemize}
        \item Thermal bridges validation (mini‑app/demonstrator): Section~\ref{sec:app:specs:app-feelpp-discr-1}.
        \item FDA nozzle CFD validation (Navier–Stokes): Section~\ref{sec:app:specs:app-feelpp-discr-2}.
        \item Parallel I/O mini‑app: Section~\ref{sec:app:specs:app-feelpp-io}.
        \item Distance computation (FMM vs BVH): Section~\ref{sec:app:specs:app-feelpp-distance}.
        \item Reduced basis mini‑app (offline/online): Section~\ref{sec:app:specs:app-feelpp-rb}.
        \item Nonlinear compressive RB (spec‑only proposal): Section~\ref{sec:app:specs:app-feelpp-nlcrb}.
\end{itemize}
Additional proposals are summarized in Section~\ref{sec:app:specs:proposed}.
...
\begin{longtable}{lp{0.3\textwidth}p{0.5\textwidth}}
    ...
    The~\Cref{fig:feelpp-cb} shows the \Feelpp Continuous Benchmarking Workflow using GitHub Actions and Reframe which is quite close to the methodology described in~\Cref{chap:methodology}.
    \begin{figure}
        \centering
