...
C++, header-only by default\\
\end{tabular} & Programming languages and language standards supported by the software \\
        \rowcolor{numpexlightergray} Parallelism  & Multithreaded kernels via Intel oneTBB; experimental task-based GPU bridges
 & Parallel computing methods and frameworks utilized by the software. CGAL requires a compiler supporting C++17 or later, and the Intel TBB library for multithreading.\\
        \rowcolor{white}Data Formats  & OFF, OBJ, STL, PLY, Medit (.mesh), VTK, Avizo, TetGen & Data formats that the software can handle or produce. For 3D meshes, CGAL can output Medit, VTK, Avizo and Tetgen. The input can be provided in different forms: implicit or explicit. It can read surface meshes in OBJ or OFF formats.\\
\\
...
Nightly cross-platform dashboards\\
...
CTest-driven regression suites\\
22-platform pre-release farm\\
...
Apptainer recipes via Spack CI\\
Reproducible developer environments (Dev Containers)\\
\end{tabular}
