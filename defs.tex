...
% Standalone WP documents do not define a DOI; the main document overrides this.
\doiNumber{}
...
% D7.1 v3 integration-draft guidance.  These commands provide presentation only:
% the editable instructions and advance records remain in each assigned WP file.
\newcommand{\WPUpdateNote}[1]{%
\par\smallskip
\noindent{\color{gray}\small\textbf{2026 update.} #1}
\par\smallskip
}

\newcommand{\WPFieldHint}[1]{%
{\color{gray}\emph{#1}}%
}

\newenvironment{WPAdvanceRecord}[1]{%
\begin{longtable}{@{}p{0.29\textwidth}p{0.66\textwidth}@{}}
\caption{Annual advance record \texttt{#1}.}
\label{tab:advance:#1}\\
\toprule
...
% Compact metadata block for application specifications introduced during the
% v3 integration pass.  Scientific detail remains an explicit owner TODO until
% a reviewed specification and versioned evidence are available.
\newcommand{\VThreeApplicationDraft}[8]{%
\section{Application: \texttt{#1}}
\label{sec:app:specs:#1}
...
scientific specification & \todo{Owner review and versioned specification required.} \\
software version/commit & \todo{Pin before reporting results.} \\
evidence/raw data & \todo{Provide a durable URL or DOI.} \\
\bottomrule
\end{longtable}
}
