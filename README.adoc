...
== Updating a Framework Fiche

The targeted Python generator imports one framework from the current
*Frameworks / Name* catalogue schema or the legacy *Software / Software*
schema. It accepts an Excel export (.xlsx), CSV, or a selected-row JSON
snapshot. Dependencies are declared in the script and installed by uv;
no project installation or bibliography update is needed.

.Reproduce the FUnTiDES metadata from the committed snapshot
[source,shell]
----
uv run gen.py --input software/funtides/catalogue.json \
  --framework FUnTiDES --output software/funtides/metadata.tex

# Verify reproducibility without writing
uv run gen.py --input software/funtides/catalogue.json \
  --framework FUnTiDES --output software/funtides/metadata.tex --check
----

.Import a new Excel export into review files
[source,shell]
----
uv run gen.py --input /path/to/catalogue.xlsx --framework FUnTiDES \
  --snapshot /tmp/funtides-new.json --output /tmp/funtides-metadata.tex
----

For Excel input, the generator chooses Frameworks first, then Software.
Use --sheet to override the worksheet. Names must match exactly
(case-insensitive); absent or duplicate matches are errors.

.Fetch a Google Sheets tab through its read-only CSV export
[source,shell]
----
uv run gen.py \
  --sheet-url 'https://docs.google.com/spreadsheets/d/19v57jpek52nQV2V0tBBON5ivGCz7Bqf3Gw-fHroVHkA/edit?gid=0' \
  --sheet Frameworks --framework FUnTiDES \
  --snapshot /tmp/funtides-live.json --output /tmp/funtides-live.tex
----

The direct export needs unauthenticated read access. For a private sheet,
download an Excel/CSV export from your authenticated Google session, or
use an authenticated connector to obtain the selected row. Do not change
the sheet's sharing permissions for the generator. The command stores no
credentials and never writes to Google Sheets.

Review the new snapshot before replacing the committed one. Only the
selected framework is retained, with its source, row and retrieval date.
Snapshots must be written to a new path; existing snapshots are protected.
Omitting --output prints LaTeX to standard output.

The generated *metadata.tex* is separate from the editable scientific text
in *software/funtides/funtides.tex*. The generator refuses to overwrite a
file without its generated-file marker. It never rewrites chapter indexes,
other framework fiches, application records, WP benchmark sections or
statistics. Empty fields remain unspecified and unchecked WP flags are
not converted into completed benchmarking campaigns.
Description and Comments cells remain in the JSON snapshot for editorial
review; they are not injected into the scientific prose automatically.
The original bulk generator is preserved in *scripts/legacy_gen.py* for
reference only: it overwrites editorial content and should not be used
for routine updates.

.Tests and developer-review PDF
[source,shell]
----
uv run --no-project --with 'jinja2>=3.1,<4' --with 'openpyxl>=3.1,<4' \
  --with pytest python -m pytest tests/test_gen.py -q
latexmk -norc --shell-escape -pdf -halt-on-error standalone-funtides.tex
----

The framework CI workflow checks the generator and publishes the standalone
fiche as the *funtides-framework-preview* artifact. The normal LaTeX workflow
also compiles the fiche inside the full deliverable.
No application or benchmark association is introduced by this first fiche.
...
The project uses Zotero group `5582837` as its shared bibliography.
The `numpex/exama` repository retrieves HAL records and files them in Zotero's
`Our Publications/WP1` through `WP7` collections. This repository only retrieves
the bibliography; it does not import HAL records or assign work packages.

The scheduled `bibtex.yml` workflow retrieves references daily at *05:17 UTC*,
one hour after the Exa-MA HAL-to-Zotero job. Both workflows also support manual
dispatch. Schedules run from the default branch and may be delayed by GitHub;
the time offset is not a dependency between workflows.

Set the repository or organization Actions secret `ZOTERO_API_KEY` for this
repository. Read access to the group is sufficient here; the `exama` importer
requires write access. The secret value is never stored in the repository.

Retrieval preserves existing citation keys, matches records by Zotero item ID,
DOI, URL, or title/year, and collapses identical exports. Ambiguous matches
retain the existing entry and appear in the `bibliography-report` artifact.
Unmatched local entries remain available, including manually added references.
The `zotero-key` BibTeX field records identity for subsequent updates.
Empty Zotero placeholders are skipped. Different records sharing an export key
receive a stable Zotero-item suffix when necessary.

The file is replaced atomically only after a complete, consistent, validated
download. Unchanged entries produce no commit. The workflow commits only
`references.bib`; branch rules must allow its `GITHUB_TOKEN` to push. Such commits
do not trigger another Actions build. LaTeX builds use the committed bibliography
snapshot without contacting Zotero.

=== Updating References

To update `references.bib`, you have several options:

==== Option 1: Using the validated repository exporter (Recommended)

[source,console]
----
export ZOTERO_API_KEY=your_api_key_here
uv run --no-project --python 3.11 --with bibtexparser==1.4.3 \
  python scripts/update_bibliography.py --check --report /tmp/bibliography-report.json
# Remove --check to update references.bib after inspecting the report.
# The legacy entry point now invokes the same exporter:
./a.cli update-bibtex
----
...
Direct GUI or `article-cli update-bibtex` exports do not perform this repository's
citation-key reconciliation. Avoid replacing the document bibliography with
them without checking that all existing citation keys are preserved.
...
2. **Bibliography Snapshot**: Uses committed references, refreshed by the daily Zotero workflow
