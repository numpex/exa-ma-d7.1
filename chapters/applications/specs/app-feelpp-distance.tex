...
This mini-app computes signed distance fields on unstructured meshes, a fundamental operation in level-set methods, shape optimization, and computational geometry.
Unlike the PDE solver demonstrators in \Cref{sec:app:specs:app-feelpp-discr-1,sec:app:specs:app-feelpp-discr-2}, it compares algorithmic approaches (Fast Marching Method vs. BVH-based ray tracing) rather than discretization methods.
...
\begin{table}[ht]
    ...
    \begin{tblr}{
        ...
        % inputs &  \\
        % outputs & \\
        metrics & \texttt{analytical-solution-match}, \texttt{strong-scalability}, \texttt{weak-scalability} \\
        status & benchmark-ready (operational Sheet status; see evidence stage) \\
        Benchmark scope & multi-node, full-system, solver-scaling \\
        Framework & Feel++ \\
...
For this specific plot, we took $h=0.06$ and $M=500$.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{Numerical results of distance computing, with $h=0.06$ and $M=500$.}
...
\begin{figure}
  ...
  \begin{subfigure}{0.49\textwidth}
    ...
    \begin{axis}[
      ...
      \addplot+[color=black, dashed, mark=none, line width=1pt] table[x=h, y={create col/linear regression={y=100}}]{\dataConvBVH};
