...
Software release, commit or PR & URANIE releases v4.10.0 and v4.11.0 \\
...
\textbf{Advance 1 (T6.2):} The methodology proposed in \cite{nassouradine2026} addresses the simultaneous prediction of multiple high-dimensional physical fields that are linked by linear equality constraints. It is shown that the usual strategy of deducing one output from the others through the linear equality constraint is sensitive to the arbitrary choice of which output is deduced, degrading both predictive accuracy and uncertainty quantification. The proposed approach instead treats all fields symmetrically: a well-tailored PCA reduces the dimension of the multi-field data while preserving the constraint in the latent space, and a theoretical analysis gives conditions under which this choice incurs a negligible reconstruction cost. A linearly-constrained multi-output Gaussian process, based on a specific kernel parametrization, is then trained in this latent space. Applied to a population dynamics problem and to an RANS CFD case predicting Reynolds stress tensor components under the incompressibility constraint, the approach achieves competitive predictive performance while guaranteeing strict satisfaction of the physical constraint, providing a reliable and physically consistent surrogate for costly field simulations.

\textbf{Advance 2 (T6.3):} The methodology proposed in \cite{sire2025} develops a hierarchical Bayesian calibration framework for multi-output simulators, where experimental data are available only for some outputs while predictions are required for unobserved ones. A joint model-error representation is introduced to consistently propagate the discrepancy across all outputs, with additional hierarchical parameters accounting for uncertainty in the calibration variables. Applied to a Taylor cylinder impact simulation, the approach improves uncertainty quantification in transposition problems, while requiring only a small number of experimental observations and offering a computationally efficient framework suitable for large-scale simulation studies

\textbf{Advance 3 (T6.3):} The methodology proposed in \cite{lartaud2026} develops goal-oriented sequential design strategies for Bayesian inverse problems, aiming to efficiently construct surrogate models under limited computational budgets. Two approaches are proposed: CSQ focuses the design around the maximum a posteriori estimate, while IP-SUR selects new simulations according to their expected reduction of posterior-weighted prediction uncertainty. The latter is tractable for Gaussian-process surrogates and comes with theoretical convergence guarantees, while both strategies improve surrogate accuracy compared with standard designs, enabling more efficient use of computational resources in large-scale inverse problems.

\textbf{Advance 4 (T6.3):} The methodology proposed in \cite{abboud2026} aims at solving multi-physics problems in which several phsyics solvers are strongly coupled. The methodology replaces those costly physics solvers with Gaussian-process surrogate models, which are then coupled to reproduce the multiphysics behavior while rigorously propagating uncertainties. A theoretical analysis guarantees that the predictive variance remains bounded throughout the coupling process which may involve many iterations. Applied to fuel assembly bow, the approach enables uncertainty quantification and sensitivity analysis at much lower computational cost, making large-scale, massively parallel studies particularly well suited to high-performance computing.
...
Between late 2025 and September 2026, two releases of the URANIE platform were delivered: v4.10.0 and v4.11.0.

\paragraph{URANIE v4.10.0.}
\begin{itemize}
	\item \textbf{Methods development:}
	\begin{itemize}
		\item Added the \texttt{Sensitivity::TCramerVonMises} class to compute Cram\'er--von Mises indices.
		\item Added a module to \texttt{TMetropHasting} to continue an existing Metropolis--Hastings computation for a given number of additional iterations, with minor adjustments to ease restarting the \texttt{TMetropHasting} constructor with cleared \texttt{tds} attributes.
		\item \texttt{DataServer} can now read (\texttt{fileDataReadCSV}) and export (\texttt{exportDataCSV}) CSV files.
		\item Added an Ishigami binary (mainly for training sessions).
		\item Relauncher: improved support for launching complex codes (file sharing between codes, MPI-parallel code calls); beta version, not yet documented.
		\item Launcher: use of Boost.Process, which should allow better scaling with a large number of CPUs.
	\end{itemize}
	\item \textbf{Corrections and modifications:} correction and improvement of the \texttt{THSIC} class.
	\item \textbf{User manual:} added a macro for \texttt{TCramerVonMises}.
\end{itemize}

\paragraph{URANIE v4.11.0.}
\begin{itemize}
	\item \textbf{Methods development:}
	\begin{itemize}
		\item MCMC development to improve chain convergence and analysis: sequential multistart, Metropolis--Hastings (classic and component-wise), Gelman--Rubin and ESS diagnostics, multi-chain plots.
		\item Generalized use of optimization solvers, allowing for example the use of CMA-ES for the construction of kriging models.
		\item Beta version of the ReLU activation function for artificial neural networks (CPU only).
	\end{itemize}
	\item \textbf{Corrections and modifications:}
	\begin{itemize}
		\item The CIRCE method is now integrated into the calibration module.
		\item Calibration: methods renamed for consistency.
		\item Fixed a segmentation fault (\texttt{segvec}) when using kriging with a trend in multistart multithreaded mode.
	\end{itemize}
	\item \textbf{User manual:} updated calibration module documentation.
\end{itemize}
