...
% Duplicate this record in WP2.tex for each main 2026 advance and replace the
...
\WPUpdateNote{Update the inherited advances with the work performed in 2026.
Separate new achievements from the 2024--2025 baseline and cite the corresponding
annual advance records.}
% • Advance 1 (T2.1): …
% • Advance 2 (T2.2): …
% • Advance 3 (T2.3): …
% • etc.
...
\WPUpdateNote{Update the Scimba and Feel++ contributions, versions and supporting
evidence for 2026.}

Scimba is a  library that implements PINNs or  time-sequential methods
...
approximations,  etc.
In addition, the code also implements various optimization algorithms.
Moreover, it also implements continuous and discontinuous finite element solvers
on unstructured quadrangle meshes,
as well as their time-dependent counterparts.
The code is written in Python and uses Jax for automatic differentiation
(making all solvers differentiable) and GPU acceleration.
The PyTorch version of Scimba is maintained but no longer improved,
and further development will focus on the Jax version.

\subsection{Additional Software Framework}
\WPUpdateNote{Update this subsection only for additional software that supported
an identified 2026 advance.}
